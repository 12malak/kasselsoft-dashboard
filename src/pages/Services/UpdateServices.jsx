...
const UpdateServices = () => {
  ...
  useEffect(() => {
    if (servicesId) {
      const fetchData = async () => {
        try {
          const response = await axios.get(`${API_URL}/services/getbyid/${servicesId}`);
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      await axios.put(
        `${API_URL}/services/update/${lang}/${servicesId}`,
        formData,
        {
          headers: {
  ...
  return (
    <Box m="20px">
      <Header title={lang ==="ar" ? "تعديل الخدمات" :"UPDATE service "} subtitle={lang ==="ar" ? "تعديل بيانات الخدمات" : "Update an Existing service"} />

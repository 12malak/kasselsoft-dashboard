...
const UpdateBlackTerms = () => {
  const isNonMobile = useMediaQuery("(min-width:600px)");
  const { lang } = useParams();
  const navigate = useNavigate();
  const location = useLocation();
  const API_URL = process.env.REACT_APP_API_URL;
  const [blacktermsId, setblacktermsId] = useState("");
  ...
  // Fetch data when blacktermsId changes
  useEffect(() => {
    if (blacktermsId) {
      const fetchData = async () => {
        try {
          const response = await axios.get(
            `${API_URL}/termsandconditions/data/black/getbyid/${blacktermsId}`
          );
          setblackterms(response.data[0]);
        } catch (err) {
  ...
  // Set blacktermsId from location state
  useEffect(() => {
    if (location.state && location.state.id) {
      setblacktermsId(location.state.id);
    } else {
  ...
  return (
    <Box m="20px">
      <Header
        title={lang === "ar" ? "تعديل شروط" : "UPDATE Black Terms "}
        subtitle={
          lang === "ar" ? "تعديل بيانات الشروط" : "Update an Black Terms"
        }
      />
      ...
      <Formik
        enableReinitialize={true} // Important to reinitialize when blackterms changes
        initialValues={{
...
export default UpdateBlackTerms;
